		...
		return new _rsvp2.default.Promise((resolve, reject) => {
			...
			this.graph.search(search, (err, triplets) => {
				...
				let loadNodes = triplets.map(triplet => {
					return new _node2.default(this, triplet.subject).load();
				});
				_rsvp2.default.all(loadNodes).then(nodes => {
		...
		return new _rsvp2.default.Promise((resolve, reject) => {
			...
			this._fetch(triplet => {
				...
				let nodeProps = nodes.map(node => {
					let nodeNs = _util2.default.format('__props:node:%s', node);
					return new _leveldbprops2.default(this.db).map(nodeNs);
				});
				let edgeProps = edges.map(edge => {
					let propsNs = _util2.default.format('__props:edge:%s:%s:%s', edge.subject, edge.predicate, edge.object);
