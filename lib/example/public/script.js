...
searchParts.forEach(sp => {
	sp = sp.split('=');qParams[sp[0]] = sp[1];
});
...
let nodeStyle = cytoscape.stylesheet().selector('node').css({
	height: 40,
	width: 40,
...
loadGraph(qParams.name).then(elements => {
	...
	drawGraph = () => {
		...
		let cy = cytoscape(opts).on('tap', 'node', selectElem).on('tap', 'edge', selectElem).on('tap', e => {
			if (e.cyTarget === cy) cy.elements().removeClass('faded');
		});
